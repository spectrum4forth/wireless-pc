//---------------------------------------------------------------------------

#ifndef Unit2H
#define Unit2H
//---------------------------------------------------------------------------
#include <Classes.hpp>
#include <Controls.hpp>
#include <StdCtrls.hpp>
#include <Forms.hpp>
#include <Dialogs.hpp>
#include <ExtCtrls.hpp>
#include <Menus.hpp>
#include <ComCtrls.hpp>
#include <Graphics.hpp>
#include <jpeg.hpp>
//---------------------------------------------------------------------------
class TForm1 : public TForm
{
__published:	// IDE-managed Components
        TButton *SendAll;
        TEdit *Edit1;
        TEdit *Edit2;
        TEdit *Edit3;
        TEdit *Edit4;
        TMainMenu *MainMenu1;
        TMenuItem *File1;
        TMenuItem *Open1;
        TMenuItem *Save1;
        TOpenDialog *OpenDialog1;
        TSaveDialog *SaveDialog1;
        TTimer *Timer1;
        TMenuItem *Convertion;
        TMenuItem *Help2;
        TPageControl *PageControl1;
        TTabSheet *TabSheet1;
        TTabSheet *TabSheet2;
        TTabSheet *TabSheet3;
        TTabSheet *TabSheet4;
        TTabSheet *TabSheet5;
        TGroupBox *GroupBox2;
        TLabel *Label17;
        TLabel *Label18;
        TLabel *Label19;
        TLabel *Label20;
        TLabel *Label21;
        TLabel *Label22;
        TLabel *Label23;
        TLabel *Label24;
        TLabel *Label25;
        TLabel *Label26;
        TLabel *Label27;
        TLabel *Label28;
        TLabel *Label29;
        TLabel *Label30;
        TLabel *Label31;
        TLabel *Label32;
        TButton *Button17;
        TButton *Button18;
        TButton *Button19;
        TButton *Button20;
        TButton *Button21;
        TButton *Button22;
        TButton *Button23;
        TButton *Button24;
        TButton *Button25;
        TButton *Button26;
        TButton *Button27;
        TButton *Button28;
        TButton *Button29;
        TButton *Button30;
        TButton *Button31;
        TButton *Button32;
        TGroupBox *GroupBox3;
        TLabel *Label33;
        TLabel *Label34;
        TLabel *Label35;
        TLabel *Label36;
        TLabel *Label37;
        TLabel *Label38;
        TLabel *Label39;
        TLabel *Label40;
        TLabel *Label41;
        TLabel *Label42;
        TLabel *Label43;
        TLabel *Label44;
        TLabel *Label45;
        TLabel *Label46;
        TLabel *Label47;
        TLabel *Label48;
        TButton *Button33;
        TButton *Button34;
        TButton *Button35;
        TButton *Button36;
        TButton *Button37;
        TButton *Button38;
        TButton *Button39;
        TButton *Button40;
        TButton *Button41;
        TButton *Button42;
        TButton *Button43;
        TButton *Button44;
        TButton *Button45;
        TButton *Button46;
        TButton *Button47;
        TButton *Button48;
        TGroupBox *GroupBox4;
        TLabel *Label49;
        TLabel *Label50;
        TLabel *Label51;
        TLabel *Label52;
        TLabel *Label53;
        TLabel *Label54;
        TLabel *Label55;
        TLabel *Label56;
        TLabel *Label57;
        TLabel *Label58;
        TLabel *Label59;
        TLabel *Label60;
        TLabel *Label61;
        TLabel *Label62;
        TLabel *Label63;
        TLabel *Label64;
        TButton *Button49;
        TButton *Button50;
        TButton *Button51;
        TButton *Button52;
        TButton *Button53;
        TButton *Button54;
        TButton *Button55;
        TButton *Button56;
        TButton *Button57;
        TButton *Button58;
        TButton *Button59;
        TButton *Button60;
        TButton *Button61;
        TButton *Button62;
        TButton *Button63;
        TButton *Button64;
        TGroupBox *GroupBox5;
        TLabel *Label65;
        TLabel *Label66;
        TLabel *Label67;
        TLabel *Label68;
        TLabel *Label69;
        TLabel *Label70;
        TLabel *Label71;
        TLabel *Label72;
        TLabel *Label73;
        TLabel *Label74;
        TLabel *Label75;
        TLabel *Label76;
        TLabel *Label77;
        TLabel *Label78;
        TLabel *Label79;
        TLabel *Label80;
        TButton *Button65;
        TButton *Button66;
        TButton *Button67;
        TButton *Button68;
        TButton *Button69;
        TButton *Button70;
        TButton *Button71;
        TButton *Button72;
        TButton *Button73;
        TButton *Button74;
        TButton *Button75;
        TButton *Button76;
        TButton *Button77;
        TButton *Button78;
        TButton *Button79;
        TButton *Button80;
        TGroupBox *GroupBox6;
        TLabel *Label81;
        TLabel *Label82;
        TLabel *Label83;
        TLabel *Label84;
        TLabel *Label85;
        TLabel *Label86;
        TLabel *Label87;
        TLabel *Label88;
        TLabel *Label89;
        TLabel *Label90;
        TLabel *Label91;
        TLabel *Label92;
        TLabel *Label93;
        TLabel *Label94;
        TLabel *Label95;
        TLabel *Label96;
        TButton *Button81;
        TButton *Button82;
        TButton *Button83;
        TButton *Button84;
        TButton *Button85;
        TButton *Button86;
        TButton *Button87;
        TButton *Button88;
        TButton *Button89;
        TButton *Button90;
        TButton *Button91;
        TButton *Button92;
        TButton *Button93;
        TButton *Button94;
        TButton *Button95;
        TButton *Button96;
        TGroupBox *GroupBox7;
        TLabel *Label97;
        TLabel *Label98;
        TLabel *Label99;
        TLabel *Label100;
        TLabel *Label101;
        TLabel *Label102;
        TLabel *Label103;
        TLabel *Label104;
        TLabel *Label105;
        TLabel *Label106;
        TLabel *Label107;
        TLabel *Label108;
        TLabel *Label109;
        TLabel *Label110;
        TLabel *Label111;
        TLabel *Label112;
        TButton *Button97;
        TButton *Button98;
        TButton *Button99;
        TButton *Button100;
        TButton *Button101;
        TButton *Button102;
        TButton *Button103;
        TButton *Button104;
        TButton *Button105;
        TButton *Button106;
        TButton *Button107;
        TButton *Button108;
        TButton *Button109;
        TButton *Button110;
        TButton *Button111;
        TButton *Button112;
        TGroupBox *GroupBox8;
        TLabel *Label113;
        TLabel *Label114;
        TLabel *Label115;
        TLabel *Label116;
        TLabel *Label117;
        TLabel *Label118;
        TLabel *Label119;
        TLabel *Label120;
        TLabel *Label121;
        TLabel *Label122;
        TLabel *Label123;
        TLabel *Label124;
        TLabel *Label125;
        TLabel *Label126;
        TLabel *Label127;
        TLabel *Label128;
        TButton *Button113;
        TButton *Button114;
        TButton *Button115;
        TButton *Button116;
        TButton *Button117;
        TButton *Button118;
        TButton *Button119;
        TButton *Button120;
        TButton *Button121;
        TButton *Button122;
        TButton *Button123;
        TButton *Button124;
        TButton *Button125;
        TButton *Button126;
        TButton *Button127;
        TButton *Button128;
        TGroupBox *GroupBox9;
        TLabel *Label129;
        TLabel *Label130;
        TLabel *Label131;
        TLabel *Label132;
        TLabel *Label133;
        TLabel *Label134;
        TLabel *Label135;
        TLabel *Label136;
        TLabel *Label137;
        TLabel *Label138;
        TLabel *Label139;
        TLabel *Label140;
        TLabel *Label141;
        TLabel *Label142;
        TLabel *Label143;
        TLabel *Label144;
        TButton *Button129;
        TButton *Button130;
        TButton *Button131;
        TButton *Button132;
        TButton *Button133;
        TButton *Button134;
        TButton *Button135;
        TButton *Button136;
        TButton *Button137;
        TButton *Button138;
        TButton *Button139;
        TButton *Button140;
        TButton *Button141;
        TButton *Button142;
        TButton *Button143;
        TButton *Button144;
        TGroupBox *GroupBox10;
        TLabel *Label145;
        TLabel *Label146;
        TLabel *Label147;
        TLabel *Label148;
        TLabel *Label149;
        TLabel *Label150;
        TLabel *Label151;
        TLabel *Label152;
        TLabel *Label153;
        TLabel *Label154;
        TLabel *Label155;
        TLabel *Label156;
        TLabel *Label157;
        TLabel *Label158;
        TLabel *Label159;
        TLabel *Label160;
        TButton *Button145;
        TButton *Button146;
        TButton *Button147;
        TButton *Button148;
        TButton *Button149;
        TButton *Button150;
        TButton *Button151;
        TButton *Button152;
        TButton *Button153;
        TButton *Button154;
        TButton *Button155;
        TButton *Button156;
        TButton *Button157;
        TButton *Button158;
        TButton *Button159;
        TButton *Button160;
        TGroupBox *GroupBox12;
        TLabel *Label177;
        TLabel *Label178;
        TLabel *Label179;
        TLabel *Label180;
        TLabel *Label181;
        TLabel *Label182;
        TLabel *Label183;
        TLabel *Label184;
        TLabel *Label185;
        TLabel *Label186;
        TLabel *Label187;
        TLabel *Label188;
        TLabel *Label189;
        TLabel *Label190;
        TLabel *Label191;
        TLabel *Label192;
        TButton *Button177;
        TButton *Button178;
        TButton *Button179;
        TButton *Button180;
        TButton *Button181;
        TButton *Button182;
        TButton *Button183;
        TButton *Button184;
        TButton *Button185;
        TButton *Button186;
        TButton *Button187;
        TButton *Button188;
        TButton *Button189;
        TButton *Button190;
        TButton *Button191;
        TButton *Button192;
        TGroupBox *GroupBox13;
        TLabel *Label193;
        TLabel *Label194;
        TLabel *Label195;
        TLabel *Label196;
        TLabel *Label197;
        TLabel *Label198;
        TLabel *Label199;
        TLabel *Label200;
        TLabel *Label201;
        TLabel *Label202;
        TLabel *Label203;
        TLabel *Label204;
        TLabel *Label205;
        TLabel *Label206;
        TLabel *Label207;
        TLabel *Label208;
        TButton *Button193;
        TButton *Button194;
        TButton *Button195;
        TButton *Button196;
        TButton *Button197;
        TButton *Button198;
        TButton *Button199;
        TButton *Button200;
        TButton *Button201;
        TButton *Button202;
        TButton *Button203;
        TButton *Button204;
        TButton *Button205;
        TButton *Button206;
        TButton *Button207;
        TButton *Button208;
        TGroupBox *GroupBox14;
        TLabel *Label209;
        TLabel *Label210;
        TLabel *Label211;
        TLabel *Label212;
        TLabel *Label213;
        TLabel *Label214;
        TLabel *Label215;
        TLabel *Label216;
        TLabel *Label217;
        TLabel *Label218;
        TLabel *Label219;
        TLabel *Label220;
        TLabel *Label221;
        TLabel *Label222;
        TLabel *Label223;
        TLabel *Label224;
        TButton *Button209;
        TButton *Button210;
        TButton *Button211;
        TButton *Button212;
        TButton *Button213;
        TButton *Button214;
        TButton *Button215;
        TButton *Button216;
        TButton *Button217;
        TButton *Button218;
        TButton *Button219;
        TButton *Button220;
        TButton *Button221;
        TButton *Button222;
        TButton *Button223;
        TButton *Button224;
        TGroupBox *GroupBox15;
        TLabel *Label225;
        TLabel *Label226;
        TLabel *Label227;
        TLabel *Label228;
        TLabel *Label229;
        TLabel *Label230;
        TLabel *Label231;
        TLabel *Label232;
        TLabel *Label233;
        TLabel *Label234;
        TLabel *Label235;
        TLabel *Label236;
        TLabel *Label237;
        TLabel *Label238;
        TLabel *Label239;
        TLabel *Label240;
        TButton *Button225;
        TButton *Button226;
        TButton *Button227;
        TButton *Button228;
        TButton *Button229;
        TButton *Button230;
        TButton *Button231;
        TButton *Button232;
        TButton *Button233;
        TButton *Button234;
        TButton *Button235;
        TButton *Button236;
        TButton *Button237;
        TButton *Button238;
        TButton *Button239;
        TButton *Button240;
        TGroupBox *GroupBox16;
        TLabel *Label241;
        TLabel *Label242;
        TLabel *Label243;
        TLabel *Label244;
        TLabel *Label245;
        TLabel *Label246;
        TLabel *Label247;
        TLabel *Label248;
        TLabel *Label249;
        TLabel *Label250;
        TLabel *Label251;
        TLabel *Label252;
        TLabel *Label253;
        TLabel *Label254;
        TLabel *Label255;
        TLabel *Label256;
        TButton *Button241;
        TButton *Button242;
        TButton *Button243;
        TButton *Button244;
        TButton *Button245;
        TButton *Button246;
        TButton *Button247;
        TButton *Button248;
        TButton *Button249;
        TButton *Button250;
        TButton *Button251;
        TButton *Button252;
        TButton *Button253;
        TButton *Button254;
        TButton *Button255;
        TButton *Button256;
        TGroupBox *GroupBox17;
        TLabel *Label257;
        TLabel *Label258;
        TLabel *Label259;
        TLabel *Label260;
        TLabel *Label261;
        TLabel *Label262;
        TLabel *Label263;
        TLabel *Label264;
        TLabel *Label265;
        TLabel *Label266;
        TLabel *Label267;
        TLabel *Label268;
        TLabel *Label269;
        TLabel *Label270;
        TLabel *Label271;
        TLabel *Label272;
        TButton *Button257;
        TButton *Button258;
        TButton *Button259;
        TButton *Button260;
        TButton *Button261;
        TButton *Button262;
        TButton *Button263;
        TButton *Button264;
        TButton *Button265;
        TButton *Button266;
        TButton *Button267;
        TButton *Button268;
        TButton *Button269;
        TButton *Button270;
        TButton *Button271;
        TButton *Button272;
        TGroupBox *GroupBox18;
        TLabel *Label273;
        TLabel *Label274;
        TLabel *Label275;
        TLabel *Label276;
        TLabel *Label277;
        TLabel *Label278;
        TLabel *Label279;
        TLabel *Label280;
        TLabel *Label281;
        TLabel *Label282;
        TLabel *Label283;
        TLabel *Label284;
        TLabel *Label285;
        TLabel *Label286;
        TLabel *Label287;
        TLabel *Label288;
        TButton *Button273;
        TButton *Button274;
        TButton *Button275;
        TButton *Button276;
        TButton *Button277;
        TButton *Button278;
        TButton *Button279;
        TButton *Button280;
        TButton *Button281;
        TButton *Button282;
        TButton *Button283;
        TButton *Button284;
        TButton *Button285;
        TButton *Button286;
        TButton *Button287;
        TButton *Button288;
        TGroupBox *GroupBox19;
        TLabel *Label289;
        TLabel *Label290;
        TLabel *Label291;
        TLabel *Label292;
        TLabel *Label293;
        TLabel *Label294;
        TLabel *Label295;
        TLabel *Label296;
        TLabel *Label297;
        TLabel *Label298;
        TLabel *Label299;
        TLabel *Label300;
        TLabel *Label301;
        TLabel *Label302;
        TLabel *Label303;
        TLabel *Label304;
        TButton *Button289;
        TButton *Button290;
        TButton *Button291;
        TButton *Button292;
        TButton *Button293;
        TButton *Button294;
        TButton *Button295;
        TButton *Button296;
        TButton *Button297;
        TButton *Button298;
        TButton *Button299;
        TButton *Button300;
        TButton *Button301;
        TButton *Button302;
        TButton *Button303;
        TButton *Button304;
        TGroupBox *GroupBox20;
        TLabel *Label305;
        TLabel *Label306;
        TLabel *Label307;
        TLabel *Label308;
        TLabel *Label309;
        TLabel *Label310;
        TLabel *Label311;
        TLabel *Label312;
        TLabel *Label313;
        TLabel *Label314;
        TLabel *Label315;
        TLabel *Label316;
        TLabel *Label317;
        TLabel *Label318;
        TLabel *Label319;
        TLabel *Label320;
        TButton *Button305;
        TButton *Button306;
        TButton *Button307;
        TButton *Button308;
        TButton *Button309;
        TButton *Button310;
        TButton *Button311;
        TButton *Button312;
        TButton *Button313;
        TButton *Button314;
        TButton *Button315;
        TButton *Button316;
        TButton *Button317;
        TButton *Button318;
        TButton *Button319;
        TButton *Button320;
        TGroupBox *GroupBox1;
        TLabel *Label1;
        TLabel *Label2;
        TLabel *Label3;
        TLabel *Label4;
        TLabel *Label5;
        TLabel *Label6;
        TLabel *Label7;
        TLabel *Label8;
        TLabel *Label9;
        TLabel *Label10;
        TLabel *Label11;
        TLabel *Label12;
        TLabel *Label13;
        TLabel *Label14;
        TLabel *Label15;
        TLabel *Label16;
        TButton *Button1;
        TButton *Button2;
        TButton *Button3;
        TButton *Button4;
        TButton *Button5;
        TButton *Button6;
        TButton *Button7;
        TButton *Button8;
        TButton *Button9;
        TButton *Button10;
        TButton *Button11;
        TButton *Button12;
        TButton *Button13;
        TButton *Button14;
        TButton *Button15;
        TButton *Button16;
        TGroupBox *GroupBox11;
        TLabel *Label161;
        TLabel *Label162;
        TLabel *Label163;
        TLabel *Label164;
        TLabel *Label165;
        TLabel *Label166;
        TLabel *Label167;
        TLabel *Label168;
        TLabel *Label169;
        TLabel *Label170;
        TLabel *Label171;
        TLabel *Label172;
        TLabel *Label173;
        TLabel *Label174;
        TLabel *Label175;
        TLabel *Label176;
        TButton *Button161;
        TButton *Button162;
        TButton *Button163;
        TButton *Button164;
        TButton *Button165;
        TButton *Button166;
        TButton *Button167;
        TButton *Button168;
        TButton *Button169;
        TButton *Button170;
        TButton *Button171;
        TButton *Button172;
        TButton *Button173;
        TButton *Button174;
        TButton *Button175;
        TButton *Button176;
        TGroupBox *GroupBox21;
        TLabel *Label321;
        TLabel *Label322;
        TLabel *Label323;
        TLabel *Label324;
        TLabel *Label325;
        TLabel *Label326;
        TLabel *Label327;
        TLabel *Label328;
        TLabel *Label329;
        TLabel *Label330;
        TLabel *Label331;
        TLabel *Label332;
        TLabel *Label333;
        TLabel *Label334;
        TLabel *Label335;
        TLabel *Label336;
        TButton *Button321;
        TButton *Button322;
        TButton *Button323;
        TButton *Button324;
        TButton *Button325;
        TButton *Button326;
        TButton *Button327;
        TButton *Button328;
        TButton *Button329;
        TButton *Button330;
        TButton *Button331;
        TButton *Button332;
        TButton *Button333;
        TButton *Button334;
        TButton *Button335;
        TButton *Button336;
        TGroupBox *GroupBox22;
        TLabel *Label337;
        TLabel *Label338;
        TLabel *Label339;
        TLabel *Label340;
        TLabel *Label341;
        TLabel *Label342;
        TLabel *Label343;
        TLabel *Label344;
        TLabel *Label345;
        TLabel *Label346;
        TLabel *Label347;
        TLabel *Label348;
        TLabel *Label349;
        TLabel *Label350;
        TLabel *Label351;
        TLabel *Label352;
        TButton *Button337;
        TButton *Button338;
        TButton *Button339;
        TButton *Button340;
        TButton *Button341;
        TButton *Button342;
        TButton *Button343;
        TButton *Button344;
        TButton *Button345;
        TButton *Button346;
        TButton *Button347;
        TButton *Button348;
        TButton *Button349;
        TButton *Button350;
        TButton *Button351;
        TButton *Button352;
        TGroupBox *GroupBox23;
        TLabel *Label353;
        TLabel *Label354;
        TLabel *Label355;
        TLabel *Label356;
        TLabel *Label357;
        TLabel *Label358;
        TLabel *Label359;
        TLabel *Label360;
        TLabel *Label361;
        TLabel *Label362;
        TLabel *Label363;
        TLabel *Label364;
        TLabel *Label365;
        TLabel *Label366;
        TLabel *Label367;
        TLabel *Label368;
        TButton *Button353;
        TButton *Button354;
        TButton *Button355;
        TButton *Button356;
        TButton *Button357;
        TButton *Button358;
        TButton *Button359;
        TButton *Button360;
        TButton *Button361;
        TButton *Button362;
        TButton *Button363;
        TButton *Button364;
        TButton *Button365;
        TButton *Button366;
        TButton *Button367;
        TButton *Button368;
        TGroupBox *GroupBox24;
        TLabel *Label369;
        TLabel *Label370;
        TLabel *Label371;
        TLabel *Label372;
        TLabel *Label373;
        TLabel *Label374;
        TLabel *Label375;
        TLabel *Label376;
        TLabel *Label377;
        TLabel *Label378;
        TLabel *Label379;
        TLabel *Label380;
        TLabel *Label381;
        TLabel *Label382;
        TLabel *Label383;
        TLabel *Label384;
        TButton *Button369;
        TButton *Button370;
        TButton *Button371;
        TButton *Button372;
        TButton *Button373;
        TButton *Button374;
        TButton *Button375;
        TButton *Button376;
        TButton *Button377;
        TButton *Button378;
        TButton *Button379;
        TButton *Button380;
        TButton *Button381;
        TButton *Button382;
        TButton *Button383;
        TButton *Button384;
        TGroupBox *GroupBox25;
        TLabel *Label385;
        TLabel *Label386;
        TLabel *Label387;
        TLabel *Label388;
        TLabel *Label389;
        TLabel *Label390;
        TLabel *Label391;
        TLabel *Label392;
        TLabel *Label393;
        TLabel *Label394;
        TLabel *Label395;
        TLabel *Label396;
        TLabel *Label397;
        TLabel *Label398;
        TLabel *Label399;
        TLabel *Label400;
        TButton *Button385;
        TButton *Button386;
        TButton *Button387;
        TButton *Button388;
        TButton *Button389;
        TButton *Button390;
        TButton *Button391;
        TButton *Button392;
        TButton *Button393;
        TButton *Button394;
        TButton *Button395;
        TButton *Button396;
        TButton *Button397;
        TButton *Button398;
        TButton *Button399;
        TButton *Button400;
        TGroupBox *GroupBox26;
        TLabel *Label401;
        TLabel *Label402;
        TLabel *Label403;
        TLabel *Label404;
        TLabel *Label405;
        TLabel *Label406;
        TLabel *Label407;
        TLabel *Label408;
        TLabel *Label409;
        TLabel *Label410;
        TLabel *Label411;
        TLabel *Label412;
        TLabel *Label413;
        TLabel *Label414;
        TLabel *Label415;
        TLabel *Label416;
        TButton *Button401;
        TButton *Button402;
        TButton *Button403;
        TButton *Button404;
        TButton *Button405;
        TButton *Button406;
        TButton *Button407;
        TButton *Button408;
        TButton *Button409;
        TButton *Button410;
        TButton *Button411;
        TButton *Button412;
        TButton *Button413;
        TButton *Button414;
        TButton *Button415;
        TButton *Button416;
        TGroupBox *GroupBox27;
        TLabel *Label417;
        TLabel *Label418;
        TLabel *Label419;
        TLabel *Label420;
        TLabel *Label421;
        TLabel *Label422;
        TLabel *Label423;
        TLabel *Label424;
        TLabel *Label425;
        TLabel *Label426;
        TLabel *Label427;
        TLabel *Label428;
        TLabel *Label429;
        TLabel *Label430;
        TLabel *Label431;
        TLabel *Label432;
        TButton *Button417;
        TButton *Button418;
        TButton *Button419;
        TButton *Button420;
        TButton *Button421;
        TButton *Button422;
        TButton *Button423;
        TButton *Button424;
        TButton *Button425;
        TButton *Button426;
        TButton *Button427;
        TButton *Button428;
        TButton *Button429;
        TButton *Button430;
        TButton *Button431;
        TButton *Button432;
        TGroupBox *GroupBox28;
        TLabel *Label433;
        TLabel *Label434;
        TLabel *Label435;
        TLabel *Label436;
        TLabel *Label437;
        TLabel *Label438;
        TLabel *Label439;
        TLabel *Label440;
        TLabel *Label441;
        TLabel *Label442;
        TLabel *Label443;
        TLabel *Label444;
        TLabel *Label445;
        TLabel *Label446;
        TLabel *Label447;
        TLabel *Label448;
        TButton *Button433;
        TButton *Button434;
        TButton *Button435;
        TButton *Button436;
        TButton *Button437;
        TButton *Button438;
        TButton *Button439;
        TButton *Button440;
        TButton *Button441;
        TButton *Button442;
        TButton *Button443;
        TButton *Button444;
        TButton *Button445;
        TButton *Button446;
        TButton *Button447;
        TButton *Button448;
        TGroupBox *GroupBox29;
        TLabel *Label449;
        TLabel *Label450;
        TLabel *Label451;
        TLabel *Label452;
        TLabel *Label453;
        TLabel *Label454;
        TLabel *Label455;
        TLabel *Label456;
        TLabel *Label457;
        TLabel *Label458;
        TLabel *Label459;
        TLabel *Label460;
        TLabel *Label461;
        TLabel *Label462;
        TLabel *Label463;
        TLabel *Label464;
        TButton *Button449;
        TButton *Button450;
        TButton *Button451;
        TButton *Button452;
        TButton *Button453;
        TButton *Button454;
        TButton *Button455;
        TButton *Button456;
        TButton *Button457;
        TButton *Button458;
        TButton *Button459;
        TButton *Button460;
        TButton *Button461;
        TButton *Button462;
        TButton *Button463;
        TButton *Button464;
        TGroupBox *GroupBox30;
        TLabel *Label465;
        TLabel *Label466;
        TLabel *Label467;
        TLabel *Label468;
        TLabel *Label469;
        TLabel *Label470;
        TLabel *Label471;
        TLabel *Label472;
        TLabel *Label473;
        TLabel *Label474;
        TLabel *Label475;
        TLabel *Label476;
        TLabel *Label477;
        TLabel *Label478;
        TLabel *Label479;
        TLabel *Label480;
        TButton *Button465;
        TButton *Button466;
        TButton *Button467;
        TButton *Button468;
        TButton *Button469;
        TButton *Button470;
        TButton *Button471;
        TButton *Button472;
        TButton *Button473;
        TButton *Button474;
        TButton *Button475;
        TButton *Button476;
        TButton *Button477;
        TButton *Button478;
        TButton *Button479;
        TButton *Button480;
        TGroupBox *GroupBox41;
        TLabel *Label641;
        TLabel *Label642;
        TLabel *Label643;
        TLabel *Label644;
        TLabel *Label645;
        TLabel *Label646;
        TLabel *Label647;
        TLabel *Label648;
        TLabel *Label649;
        TLabel *Label650;
        TLabel *Label651;
        TLabel *Label652;
        TLabel *Label653;
        TLabel *Label654;
        TLabel *Label655;
        TLabel *Label656;
        TButton *Button641;
        TButton *Button642;
        TButton *Button643;
        TButton *Button644;
        TButton *Button645;
        TButton *Button646;
        TButton *Button647;
        TButton *Button648;
        TButton *Button649;
        TButton *Button650;
        TButton *Button651;
        TButton *Button652;
        TButton *Button653;
        TButton *Button654;
        TButton *Button655;
        TButton *Button656;
        TGroupBox *GroupBox42;
        TLabel *Label657;
        TLabel *Label658;
        TLabel *Label659;
        TLabel *Label660;
        TLabel *Label661;
        TLabel *Label662;
        TLabel *Label663;
        TLabel *Label664;
        TLabel *Label665;
        TLabel *Label666;
        TLabel *Label667;
        TLabel *Label668;
        TLabel *Label669;
        TLabel *Label670;
        TLabel *Label671;
        TLabel *Label672;
        TButton *Button657;
        TButton *Button658;
        TButton *Button659;
        TButton *Button660;
        TButton *Button661;
        TButton *Button662;
        TButton *Button663;
        TButton *Button664;
        TButton *Button665;
        TButton *Button666;
        TButton *Button667;
        TButton *Button668;
        TButton *Button669;
        TButton *Button670;
        TButton *Button671;
        TButton *Button672;
        TGroupBox *GroupBox43;
        TLabel *Label673;
        TLabel *Label674;
        TLabel *Label675;
        TLabel *Label676;
        TLabel *Label677;
        TLabel *Label678;
        TLabel *Label679;
        TLabel *Label680;
        TLabel *Label681;
        TLabel *Label682;
        TLabel *Label683;
        TLabel *Label684;
        TLabel *Label685;
        TLabel *Label686;
        TLabel *Label687;
        TLabel *Label688;
        TButton *Button673;
        TButton *Button674;
        TButton *Button675;
        TButton *Button676;
        TButton *Button677;
        TButton *Button678;
        TButton *Button679;
        TButton *Button680;
        TButton *Button681;
        TButton *Button682;
        TButton *Button683;
        TButton *Button684;
        TButton *Button685;
        TButton *Button686;
        TButton *Button687;
        TButton *Button688;
        TGroupBox *GroupBox44;
        TLabel *Label689;
        TLabel *Label690;
        TLabel *Label691;
        TLabel *Label692;
        TLabel *Label693;
        TLabel *Label694;
        TLabel *Label695;
        TLabel *Label696;
        TLabel *Label697;
        TLabel *Label698;
        TLabel *Label699;
        TLabel *Label700;
        TLabel *Label701;
        TLabel *Label702;
        TLabel *Label703;
        TLabel *Label704;
        TButton *Button689;
        TButton *Button690;
        TButton *Button691;
        TButton *Button692;
        TButton *Button693;
        TButton *Button694;
        TButton *Button695;
        TButton *Button696;
        TButton *Button697;
        TButton *Button698;
        TButton *Button699;
        TButton *Button700;
        TButton *Button701;
        TButton *Button702;
        TButton *Button703;
        TButton *Button704;
        TGroupBox *GroupBox45;
        TLabel *Label705;
        TLabel *Label706;
        TLabel *Label707;
        TLabel *Label708;
        TLabel *Label709;
        TLabel *Label710;
        TLabel *Label711;
        TLabel *Label712;
        TLabel *Label713;
        TLabel *Label714;
        TLabel *Label715;
        TLabel *Label716;
        TLabel *Label717;
        TLabel *Label718;
        TLabel *Label719;
        TLabel *Label720;
        TButton *Button705;
        TButton *Button706;
        TButton *Button707;
        TButton *Button708;
        TButton *Button709;
        TButton *Button710;
        TButton *Button711;
        TButton *Button712;
        TButton *Button713;
        TButton *Button714;
        TButton *Button715;
        TButton *Button716;
        TButton *Button717;
        TButton *Button718;
        TButton *Button719;
        TButton *Button720;
        TGroupBox *GroupBox46;
        TLabel *Label721;
        TLabel *Label722;
        TLabel *Label723;
        TLabel *Label724;
        TLabel *Label725;
        TLabel *Label726;
        TLabel *Label727;
        TLabel *Label728;
        TLabel *Label729;
        TLabel *Label730;
        TLabel *Label731;
        TLabel *Label732;
        TLabel *Label733;
        TLabel *Label734;
        TLabel *Label735;
        TLabel *Label736;
        TButton *Button721;
        TButton *Button722;
        TButton *Button723;
        TButton *Button724;
        TButton *Button725;
        TButton *Button726;
        TButton *Button727;
        TButton *Button728;
        TButton *Button729;
        TButton *Button730;
        TButton *Button731;
        TButton *Button732;
        TButton *Button733;
        TButton *Button734;
        TButton *Button735;
        TButton *Button736;
        TGroupBox *GroupBox47;
        TLabel *Label737;
        TLabel *Label738;
        TLabel *Label739;
        TLabel *Label740;
        TLabel *Label741;
        TLabel *Label742;
        TLabel *Label743;
        TLabel *Label744;
        TLabel *Label745;
        TLabel *Label746;
        TLabel *Label747;
        TLabel *Label748;
        TLabel *Label749;
        TLabel *Label750;
        TLabel *Label751;
        TLabel *Label752;
        TButton *Button737;
        TButton *Button738;
        TButton *Button739;
        TButton *Button740;
        TButton *Button741;
        TButton *Button742;
        TButton *Button743;
        TButton *Button744;
        TButton *Button745;
        TButton *Button746;
        TButton *Button747;
        TButton *Button748;
        TButton *Button749;
        TButton *Button750;
        TButton *Button751;
        TButton *Button752;
        TGroupBox *GroupBox48;
        TLabel *Label753;
        TLabel *Label754;
        TLabel *Label755;
        TLabel *Label756;
        TLabel *Label757;
        TLabel *Label758;
        TLabel *Label759;
        TLabel *Label760;
        TLabel *Label761;
        TLabel *Label762;
        TLabel *Label763;
        TLabel *Label764;
        TLabel *Label765;
        TLabel *Label766;
        TLabel *Label767;
        TLabel *Label768;
        TButton *Button753;
        TButton *Button754;
        TButton *Button755;
        TButton *Button756;
        TButton *Button757;
        TButton *Button758;
        TButton *Button759;
        TButton *Button760;
        TButton *Button761;
        TButton *Button762;
        TButton *Button763;
        TButton *Button764;
        TButton *Button765;
        TButton *Button766;
        TButton *Button767;
        TButton *Button768;
        TGroupBox *GroupBox49;
        TLabel *Label769;
        TLabel *Label770;
        TLabel *Label771;
        TLabel *Label772;
        TLabel *Label773;
        TLabel *Label774;
        TLabel *Label775;
        TLabel *Label776;
        TLabel *Label777;
        TLabel *Label778;
        TLabel *Label779;
        TLabel *Label780;
        TLabel *Label781;
        TLabel *Label782;
        TLabel *Label783;
        TLabel *Label784;
        TButton *Button769;
        TButton *Button770;
        TButton *Button771;
        TButton *Button772;
        TButton *Button773;
        TButton *Button774;
        TButton *Button775;
        TButton *Button776;
        TButton *Button777;
        TButton *Button778;
        TButton *Button779;
        TButton *Button780;
        TButton *Button781;
        TButton *Button782;
        TButton *Button783;
        TButton *Button784;
        TGroupBox *GroupBox50;
        TLabel *Label785;
        TLabel *Label786;
        TLabel *Label787;
        TLabel *Label788;
        TLabel *Label789;
        TLabel *Label790;
        TLabel *Label791;
        TLabel *Label792;
        TLabel *Label793;
        TLabel *Label794;
        TLabel *Label795;
        TLabel *Label796;
        TLabel *Label797;
        TLabel *Label798;
        TLabel *Label799;
        TLabel *Label800;
        TButton *Button785;
        TButton *Button786;
        TButton *Button787;
        TButton *Button788;
        TButton *Button789;
        TButton *Button790;
        TButton *Button791;
        TButton *Button792;
        TButton *Button793;
        TButton *Button794;
        TButton *Button795;
        TButton *Button796;
        TButton *Button797;
        TButton *Button798;
        TButton *Button799;
        TButton *Button800;
        TGroupBox *GroupBox61;
        TLabel *Label961;
        TLabel *Label962;
        TLabel *Label963;
        TLabel *Label964;
        TLabel *Label965;
        TLabel *Label966;
        TLabel *Label967;
        TLabel *Label968;
        TLabel *Label969;
        TLabel *Label970;
        TLabel *Label971;
        TLabel *Label972;
        TLabel *Label973;
        TLabel *Label974;
        TLabel *Label975;
        TLabel *Label976;
        TButton *Button961;
        TButton *Button962;
        TButton *Button963;
        TButton *Button964;
        TButton *Button965;
        TButton *Button966;
        TButton *Button967;
        TButton *Button968;
        TButton *Button969;
        TButton *Button970;
        TButton *Button971;
        TButton *Button972;
        TButton *Button973;
        TButton *Button974;
        TButton *Button975;
        TButton *Button976;
        TGroupBox *GroupBox62;
        TLabel *Label977;
        TLabel *Label978;
        TLabel *Label979;
        TLabel *Label980;
        TLabel *Label981;
        TLabel *Label982;
        TLabel *Label983;
        TLabel *Label984;
        TLabel *Label985;
        TLabel *Label986;
        TLabel *Label987;
        TLabel *Label988;
        TLabel *Label989;
        TLabel *Label990;
        TLabel *Label991;
        TLabel *Label992;
        TButton *Button977;
        TButton *Button978;
        TButton *Button979;
        TButton *Button980;
        TButton *Button981;
        TButton *Button982;
        TButton *Button983;
        TButton *Button984;
        TButton *Button985;
        TButton *Button986;
        TButton *Button987;
        TButton *Button988;
        TButton *Button989;
        TButton *Button990;
        TButton *Button991;
        TButton *Button992;
        TGroupBox *GroupBox63;
        TLabel *Label993;
        TLabel *Label994;
        TLabel *Label995;
        TLabel *Label996;
        TLabel *Label997;
        TLabel *Label998;
        TLabel *Label999;
        TLabel *Label1000;
        TLabel *Label1001;
        TLabel *Label1002;
        TLabel *Label1003;
        TLabel *Label1004;
        TLabel *Label1005;
        TLabel *Label1006;
        TLabel *Label1007;
        TLabel *Label1008;
        TButton *Button993;
        TButton *Button994;
        TButton *Button995;
        TButton *Button996;
        TButton *Button997;
        TButton *Button998;
        TButton *Button999;
        TButton *Button1000;
        TButton *Button1001;
        TButton *Button1002;
        TButton *Button1003;
        TButton *Button1004;
        TButton *Button1005;
        TButton *Button1006;
        TButton *Button1007;
        TButton *Button1008;
        TGroupBox *GroupBox64;
        TLabel *Label1009;
        TLabel *Label1010;
        TLabel *Label1011;
        TLabel *Label1012;
        TLabel *Label1013;
        TLabel *Label1014;
        TLabel *Label1015;
        TLabel *Label1016;
        TLabel *Label1017;
        TLabel *Label1018;
        TLabel *Label1019;
        TLabel *Label1020;
        TLabel *Label1021;
        TLabel *Label1022;
        TLabel *Label1023;
        TLabel *Label1024;
        TButton *Button1009;
        TButton *Button1010;
        TButton *Button1011;
        TButton *Button1012;
        TButton *Button1013;
        TButton *Button1014;
        TButton *Button1015;
        TButton *Button1016;
        TButton *Button1017;
        TButton *Button1018;
        TButton *Button1019;
        TButton *Button1020;
        TButton *Button1021;
        TButton *Button1022;
        TButton *Button1023;
        TButton *Button1024;
        TGroupBox *GroupBox65;
        TLabel *Label1025;
        TLabel *Label1026;
        TLabel *Label1027;
        TLabel *Label1028;
        TLabel *Label1029;
        TLabel *Label1030;
        TLabel *Label1031;
        TLabel *Label1032;
        TLabel *Label1033;
        TLabel *Label1034;
        TLabel *Label1035;
        TLabel *Label1036;
        TLabel *Label1037;
        TLabel *Label1038;
        TLabel *Label1039;
        TLabel *Label1040;
        TButton *Button1025;
        TButton *Button1026;
        TButton *Button1027;
        TButton *Button1028;
        TButton *Button1029;
        TButton *Button1030;
        TButton *Button1031;
        TButton *Button1032;
        TButton *Button1033;
        TButton *Button1034;
        TButton *Button1035;
        TButton *Button1036;
        TButton *Button1037;
        TButton *Button1038;
        TButton *Button1039;
        TButton *Button1040;
        TGroupBox *GroupBox66;
        TLabel *Label1041;
        TLabel *Label1042;
        TLabel *Label1043;
        TLabel *Label1044;
        TLabel *Label1045;
        TLabel *Label1046;
        TLabel *Label1047;
        TLabel *Label1048;
        TLabel *Label1049;
        TLabel *Label1050;
        TLabel *Label1051;
        TLabel *Label1052;
        TLabel *Label1053;
        TLabel *Label1054;
        TLabel *Label1055;
        TLabel *Label1056;
        TButton *Button1041;
        TButton *Button1042;
        TButton *Button1043;
        TButton *Button1044;
        TButton *Button1045;
        TButton *Button1046;
        TButton *Button1047;
        TButton *Button1048;
        TButton *Button1049;
        TButton *Button1050;
        TButton *Button1051;
        TButton *Button1052;
        TButton *Button1053;
        TButton *Button1054;
        TButton *Button1055;
        TButton *Button1056;
        TGroupBox *GroupBox67;
        TLabel *Label1057;
        TLabel *Label1058;
        TLabel *Label1059;
        TLabel *Label1060;
        TLabel *Label1061;
        TLabel *Label1062;
        TLabel *Label1063;
        TLabel *Label1064;
        TLabel *Label1065;
        TLabel *Label1066;
        TLabel *Label1067;
        TLabel *Label1068;
        TLabel *Label1069;
        TLabel *Label1070;
        TLabel *Label1071;
        TLabel *Label1072;
        TButton *Button1057;
        TButton *Button1058;
        TButton *Button1059;
        TButton *Button1060;
        TButton *Button1061;
        TButton *Button1062;
        TButton *Button1063;
        TButton *Button1064;
        TButton *Button1065;
        TButton *Button1066;
        TButton *Button1067;
        TButton *Button1068;
        TButton *Button1069;
        TButton *Button1070;
        TButton *Button1071;
        TButton *Button1072;
        TGroupBox *GroupBox68;
        TLabel *Label1073;
        TLabel *Label1074;
        TLabel *Label1075;
        TLabel *Label1076;
        TLabel *Label1077;
        TLabel *Label1078;
        TLabel *Label1079;
        TLabel *Label1080;
        TLabel *Label1081;
        TLabel *Label1082;
        TLabel *Label1083;
        TLabel *Label1084;
        TLabel *Label1085;
        TLabel *Label1086;
        TLabel *Label1087;
        TLabel *Label1088;
        TButton *Button1073;
        TButton *Button1074;
        TButton *Button1075;
        TButton *Button1076;
        TButton *Button1077;
        TButton *Button1078;
        TButton *Button1079;
        TButton *Button1080;
        TButton *Button1081;
        TButton *Button1082;
        TButton *Button1083;
        TButton *Button1084;
        TButton *Button1085;
        TButton *Button1086;
        TButton *Button1087;
        TButton *Button1088;
        TGroupBox *GroupBox69;
        TLabel *Label1089;
        TLabel *Label1090;
        TLabel *Label1091;
        TLabel *Label1092;
        TLabel *Label1093;
        TLabel *Label1094;
        TLabel *Label1095;
        TLabel *Label1096;
        TLabel *Label1097;
        TLabel *Label1098;
        TLabel *Label1099;
        TLabel *Label1100;
        TLabel *Label1101;
        TLabel *Label1102;
        TLabel *Label1103;
        TLabel *Label1104;
        TButton *Button1089;
        TButton *Button1090;
        TButton *Button1091;
        TButton *Button1092;
        TButton *Button1093;
        TButton *Button1094;
        TButton *Button1095;
        TButton *Button1096;
        TButton *Button1097;
        TButton *Button1098;
        TButton *Button1099;
        TButton *Button1100;
        TButton *Button1101;
        TButton *Button1102;
        TButton *Button1103;
        TButton *Button1104;
        TGroupBox *GroupBox70;
        TLabel *Label1105;
        TLabel *Label1106;
        TLabel *Label1107;
        TLabel *Label1108;
        TLabel *Label1109;
        TLabel *Label1110;
        TLabel *Label1111;
        TLabel *Label1112;
        TLabel *Label1113;
        TLabel *Label1114;
        TLabel *Label1115;
        TLabel *Label1116;
        TLabel *Label1117;
        TLabel *Label1118;
        TLabel *Label1119;
        TLabel *Label1120;
        TButton *Button1105;
        TButton *Button1106;
        TButton *Button1107;
        TButton *Button1108;
        TButton *Button1109;
        TButton *Button1110;
        TButton *Button1111;
        TButton *Button1112;
        TButton *Button1113;
        TButton *Button1114;
        TButton *Button1115;
        TButton *Button1116;
        TButton *Button1117;
        TButton *Button1118;
        TButton *Button1119;
        TButton *Button1120;
        TGroupBox *GroupBox81;
        TLabel *Label1281;
        TLabel *Label1282;
        TLabel *Label1283;
        TLabel *Label1284;
        TLabel *Label1285;
        TLabel *Label1286;
        TLabel *Label1287;
        TLabel *Label1288;
        TLabel *Label1289;
        TLabel *Label1290;
        TLabel *Label1291;
        TLabel *Label1292;
        TLabel *Label1293;
        TLabel *Label1294;
        TLabel *Label1295;
        TLabel *Label1296;
        TButton *Button1281;
        TButton *Button1282;
        TButton *Button1283;
        TButton *Button1284;
        TButton *Button1285;
        TButton *Button1286;
        TButton *Button1287;
        TButton *Button1288;
        TButton *Button1289;
        TButton *Button1290;
        TButton *Button1291;
        TButton *Button1292;
        TButton *Button1293;
        TButton *Button1294;
        TButton *Button1295;
        TButton *Button1296;
        TGroupBox *GroupBox82;
        TLabel *Label1297;
        TLabel *Label1298;
        TLabel *Label1299;
        TLabel *Label1300;
        TLabel *Label1301;
        TLabel *Label1302;
        TLabel *Label1303;
        TLabel *Label1304;
        TLabel *Label1305;
        TLabel *Label1306;
        TLabel *Label1307;
        TLabel *Label1308;
        TLabel *Label1309;
        TLabel *Label1310;
        TLabel *Label1311;
        TLabel *Label1312;
        TButton *Button1297;
        TButton *Button1298;
        TButton *Button1299;
        TButton *Button1300;
        TButton *Button1301;
        TButton *Button1302;
        TButton *Button1303;
        TButton *Button1304;
        TButton *Button1305;
        TButton *Button1306;
        TButton *Button1307;
        TButton *Button1308;
        TButton *Button1309;
        TButton *Button1310;
        TButton *Button1311;
        TButton *Button1312;
        TGroupBox *GroupBox83;
        TLabel *Label1313;
        TLabel *Label1314;
        TLabel *Label1315;
        TLabel *Label1316;
        TLabel *Label1317;
        TLabel *Label1318;
        TLabel *Label1319;
        TLabel *Label1320;
        TLabel *Label1321;
        TLabel *Label1322;
        TLabel *Label1323;
        TLabel *Label1324;
        TLabel *Label1325;
        TLabel *Label1326;
        TLabel *Label1327;
        TLabel *Label1328;
        TButton *Button1313;
        TButton *Button1314;
        TButton *Button1315;
        TButton *Button1316;
        TButton *Button1317;
        TButton *Button1318;
        TButton *Button1319;
        TButton *Button1320;
        TButton *Button1321;
        TButton *Button1322;
        TButton *Button1323;
        TButton *Button1324;
        TButton *Button1325;
        TButton *Button1326;
        TButton *Button1327;
        TButton *Button1328;
        TGroupBox *GroupBox84;
        TLabel *Label1329;
        TLabel *Label1330;
        TLabel *Label1331;
        TLabel *Label1332;
        TLabel *Label1333;
        TLabel *Label1334;
        TLabel *Label1335;
        TLabel *Label1336;
        TLabel *Label1337;
        TLabel *Label1338;
        TLabel *Label1339;
        TLabel *Label1340;
        TLabel *Label1341;
        TLabel *Label1342;
        TLabel *Label1343;
        TLabel *Label1344;
        TButton *Button1329;
        TButton *Button1330;
        TButton *Button1331;
        TButton *Button1332;
        TButton *Button1333;
        TButton *Button1334;
        TButton *Button1335;
        TButton *Button1336;
        TButton *Button1337;
        TButton *Button1338;
        TButton *Button1339;
        TButton *Button1340;
        TButton *Button1341;
        TButton *Button1342;
        TButton *Button1343;
        TButton *Button1344;
        TGroupBox *GroupBox85;
        TLabel *Label1345;
        TLabel *Label1346;
        TLabel *Label1347;
        TLabel *Label1348;
        TLabel *Label1349;
        TLabel *Label1350;
        TLabel *Label1351;
        TLabel *Label1352;
        TLabel *Label1353;
        TLabel *Label1354;
        TLabel *Label1355;
        TLabel *Label1356;
        TLabel *Label1357;
        TLabel *Label1358;
        TLabel *Label1359;
        TLabel *Label1360;
        TButton *Button1345;
        TButton *Button1346;
        TButton *Button1347;
        TButton *Button1348;
        TButton *Button1349;
        TButton *Button1350;
        TButton *Button1351;
        TButton *Button1352;
        TButton *Button1353;
        TButton *Button1354;
        TButton *Button1355;
        TButton *Button1356;
        TButton *Button1357;
        TButton *Button1358;
        TButton *Button1359;
        TButton *Button1360;
        TGroupBox *GroupBox86;
        TLabel *Label1361;
        TLabel *Label1362;
        TLabel *Label1363;
        TLabel *Label1364;
        TLabel *Label1365;
        TLabel *Label1366;
        TLabel *Label1367;
        TLabel *Label1368;
        TLabel *Label1369;
        TLabel *Label1370;
        TLabel *Label1371;
        TLabel *Label1372;
        TLabel *Label1373;
        TLabel *Label1374;
        TLabel *Label1375;
        TLabel *Label1376;
        TButton *Button1361;
        TButton *Button1362;
        TButton *Button1363;
        TButton *Button1364;
        TButton *Button1365;
        TButton *Button1366;
        TButton *Button1367;
        TButton *Button1368;
        TButton *Button1369;
        TButton *Button1370;
        TButton *Button1371;
        TButton *Button1372;
        TButton *Button1373;
        TButton *Button1374;
        TButton *Button1375;
        TButton *Button1376;
        TGroupBox *GroupBox87;
        TLabel *Label1377;
        TLabel *Label1378;
        TLabel *Label1379;
        TLabel *Label1380;
        TLabel *Label1381;
        TLabel *Label1382;
        TLabel *Label1383;
        TLabel *Label1384;
        TLabel *Label1385;
        TLabel *Label1386;
        TLabel *Label1387;
        TLabel *Label1388;
        TLabel *Label1389;
        TLabel *Label1390;
        TLabel *Label1391;
        TLabel *Label1392;
        TButton *Button1377;
        TButton *Button1378;
        TButton *Button1379;
        TButton *Button1380;
        TButton *Button1381;
        TButton *Button1382;
        TButton *Button1383;
        TButton *Button1384;
        TButton *Button1385;
        TButton *Button1386;
        TButton *Button1387;
        TButton *Button1388;
        TButton *Button1389;
        TButton *Button1390;
        TButton *Button1391;
        TButton *Button1392;
        TGroupBox *GroupBox88;
        TLabel *Label1393;
        TLabel *Label1394;
        TLabel *Label1395;
        TLabel *Label1396;
        TLabel *Label1397;
        TLabel *Label1398;
        TLabel *Label1399;
        TLabel *Label1400;
        TLabel *Label1401;
        TLabel *Label1402;
        TLabel *Label1403;
        TLabel *Label1404;
        TLabel *Label1405;
        TLabel *Label1406;
        TLabel *Label1407;
        TLabel *Label1408;
        TButton *Button1393;
        TButton *Button1394;
        TButton *Button1395;
        TButton *Button1396;
        TButton *Button1397;
        TButton *Button1398;
        TButton *Button1399;
        TButton *Button1400;
        TButton *Button1401;
        TButton *Button1402;
        TButton *Button1403;
        TButton *Button1404;
        TButton *Button1405;
        TButton *Button1406;
        TButton *Button1407;
        TButton *Button1408;
        TGroupBox *GroupBox89;
        TLabel *Label1409;
        TLabel *Label1410;
        TLabel *Label1411;
        TLabel *Label1412;
        TLabel *Label1413;
        TLabel *Label1414;
        TLabel *Label1415;
        TLabel *Label1416;
        TLabel *Label1417;
        TLabel *Label1418;
        TLabel *Label1419;
        TLabel *Label1420;
        TLabel *Label1421;
        TLabel *Label1422;
        TLabel *Label1423;
        TLabel *Label1424;
        TButton *Button1409;
        TButton *Button1410;
        TButton *Button1411;
        TButton *Button1412;
        TButton *Button1413;
        TButton *Button1414;
        TButton *Button1415;
        TButton *Button1416;
        TButton *Button1417;
        TButton *Button1418;
        TButton *Button1419;
        TButton *Button1420;
        TButton *Button1421;
        TButton *Button1422;
        TButton *Button1423;
        TButton *Button1424;
        TGroupBox *GroupBox90;
        TLabel *Label1425;
        TLabel *Label1426;
        TLabel *Label1427;
        TLabel *Label1428;
        TLabel *Label1429;
        TLabel *Label1430;
        TLabel *Label1431;
        TLabel *Label1432;
        TLabel *Label1433;
        TLabel *Label1434;
        TLabel *Label1435;
        TLabel *Label1436;
        TLabel *Label1437;
        TLabel *Label1438;
        TLabel *Label1439;
        TLabel *Label1440;
        TButton *Button1425;
        TButton *Button1426;
        TButton *Button1427;
        TButton *Button1428;
        TButton *Button1429;
        TButton *Button1430;
        TButton *Button1431;
        TButton *Button1432;
        TButton *Button1433;
        TButton *Button1434;
        TButton *Button1435;
        TButton *Button1436;
        TButton *Button1437;
        TButton *Button1438;
        TButton *Button1439;
        TButton *Button1440;
        TMemo *Memo1;
        TLabel *Label1601;
        TButton *ReadAll;
        TTabSheet *TabSheet6;
        TImage *Image1;
        TGroupBox *GroupBox31;
        TLabel *Label481;
        TLabel *Label482;
        TLabel *Label483;
        TLabel *Label484;
        TLabel *Label485;
        TLabel *Label486;
        TLabel *Label487;
        TLabel *Label488;
        TLabel *Label489;
        TLabel *Label490;
        TLabel *Label491;
        TLabel *Label492;
        TLabel *Label493;
        TLabel *Label494;
        TLabel *Label495;
        TLabel *Label496;
        TButton *Button481;
        TButton *Button482;
        TButton *Button483;
        TButton *Button484;
        TButton *Button485;
        TButton *Button486;
        TButton *Button487;
        TButton *Button488;
        TButton *Button489;
        TButton *Button490;
        TButton *Button491;
        TButton *Button492;
        TButton *Button493;
        TButton *Button494;
        TButton *Button495;
        TButton *Button496;
        TGroupBox *GroupBox32;
        TLabel *Label497;
        TLabel *Label498;
        TLabel *Label499;
        TLabel *Label500;
        TLabel *Label501;
        TLabel *Label502;
        TLabel *Label503;
        TLabel *Label504;
        TLabel *Label505;
        TLabel *Label506;
        TLabel *Label507;
        TLabel *Label508;
        TLabel *Label509;
        TLabel *Label510;
        TLabel *Label511;
        TLabel *Label512;
        TButton *Button497;
        TButton *Button498;
        TButton *Button499;
        TButton *Button500;
        TButton *Button501;
        TButton *Button502;
        TButton *Button503;
        TButton *Button504;
        TButton *Button505;
        TButton *Button506;
        TButton *Button507;
        TButton *Button508;
        TButton *Button509;
        TButton *Button510;
        TButton *Button511;
        TButton *Button512;
        TGroupBox *GroupBox33;
        TLabel *Label513;
        TLabel *Label514;
        TLabel *Label515;
        TLabel *Label516;
        TLabel *Label517;
        TLabel *Label518;
        TLabel *Label519;
        TLabel *Label520;
        TLabel *Label521;
        TLabel *Label522;
        TLabel *Label523;
        TLabel *Label524;
        TLabel *Label525;
        TLabel *Label526;
        TLabel *Label527;
        TLabel *Label528;
        TButton *Button513;
        TButton *Button514;
        TButton *Button515;
        TButton *Button516;
        TButton *Button517;
        TButton *Button518;
        TButton *Button519;
        TButton *Button520;
        TButton *Button521;
        TButton *Button522;
        TButton *Button523;
        TButton *Button524;
        TButton *Button525;
        TButton *Button526;
        TButton *Button527;
        TButton *Button528;
        TGroupBox *GroupBox34;
        TLabel *Label529;
        TLabel *Label530;
        TLabel *Label531;
        TLabel *Label532;
        TLabel *Label533;
        TLabel *Label534;
        TLabel *Label535;
        TLabel *Label536;
        TLabel *Label537;
        TLabel *Label538;
        TLabel *Label539;
        TLabel *Label540;
        TLabel *Label541;
        TLabel *Label542;
        TLabel *Label543;
        TLabel *Label544;
        TButton *Button529;
        TButton *Button530;
        TButton *Button531;
        TButton *Button532;
        TButton *Button533;
        TButton *Button534;
        TButton *Button535;
        TButton *Button536;
        TButton *Button537;
        TButton *Button538;
        TButton *Button539;
        TButton *Button540;
        TButton *Button541;
        TButton *Button542;
        TButton *Button543;
        TButton *Button544;
        TGroupBox *GroupBox35;
        TLabel *Label545;
        TLabel *Label546;
        TLabel *Label547;
        TLabel *Label548;
        TLabel *Label549;
        TLabel *Label550;
        TLabel *Label551;
        TLabel *Label552;
        TLabel *Label553;
        TLabel *Label554;
        TLabel *Label555;
        TLabel *Label556;
        TLabel *Label557;
        TLabel *Label558;
        TLabel *Label559;
        TLabel *Label560;
        TButton *Button545;
        TButton *Button546;
        TButton *Button547;
        TButton *Button548;
        TButton *Button549;
        TButton *Button550;
        TButton *Button551;
        TButton *Button552;
        TButton *Button553;
        TButton *Button554;
        TButton *Button555;
        TButton *Button556;
        TButton *Button557;
        TButton *Button558;
        TButton *Button559;
        TButton *Button560;
        TGroupBox *GroupBox36;
        TLabel *Label561;
        TLabel *Label562;
        TLabel *Label563;
        TLabel *Label564;
        TLabel *Label565;
        TLabel *Label566;
        TLabel *Label567;
        TLabel *Label568;
        TLabel *Label569;
        TLabel *Label570;
        TLabel *Label571;
        TLabel *Label572;
        TLabel *Label573;
        TLabel *Label574;
        TLabel *Label575;
        TLabel *Label576;
        TButton *Button561;
        TButton *Button562;
        TButton *Button563;
        TButton *Button564;
        TButton *Button565;
        TButton *Button566;
        TButton *Button567;
        TButton *Button568;
        TButton *Button569;
        TButton *Button570;
        TButton *Button571;
        TButton *Button572;
        TButton *Button573;
        TButton *Button574;
        TButton *Button575;
        TButton *Button576;
        TGroupBox *GroupBox37;
        TLabel *Label577;
        TLabel *Label578;
        TLabel *Label579;
        TLabel *Label580;
        TLabel *Label581;
        TLabel *Label582;
        TLabel *Label583;
        TLabel *Label584;
        TLabel *Label585;
        TLabel *Label586;
        TLabel *Label587;
        TLabel *Label588;
        TLabel *Label589;
        TLabel *Label590;
        TLabel *Label591;
        TLabel *Label592;
        TButton *Button577;
        TButton *Button578;
        TButton *Button579;
        TButton *Button580;
        TButton *Button581;
        TButton *Button582;
        TButton *Button583;
        TButton *Button584;
        TButton *Button585;
        TButton *Button586;
        TButton *Button587;
        TButton *Button588;
        TButton *Button589;
        TButton *Button590;
        TButton *Button591;
        TButton *Button592;
        TGroupBox *GroupBox38;
        TLabel *Label593;
        TLabel *Label594;
        TLabel *Label595;
        TLabel *Label596;
        TLabel *Label597;
        TLabel *Label598;
        TLabel *Label599;
        TLabel *Label600;
        TLabel *Label601;
        TLabel *Label602;
        TLabel *Label603;
        TLabel *Label604;
        TLabel *Label605;
        TLabel *Label606;
        TLabel *Label607;
        TLabel *Label608;
        TButton *Button593;
        TButton *Button594;
        TButton *Button595;
        TButton *Button596;
        TButton *Button597;
        TButton *Button598;
        TButton *Button599;
        TButton *Button600;
        TButton *Button601;
        TButton *Button602;
        TButton *Button603;
        TButton *Button604;
        TButton *Button605;
        TButton *Button606;
        TButton *Button607;
        TButton *Button608;
        TGroupBox *GroupBox39;
        TLabel *Label609;
        TLabel *Label610;
        TLabel *Label611;
        TLabel *Label612;
        TLabel *Label613;
        TLabel *Label614;
        TLabel *Label615;
        TLabel *Label616;
        TLabel *Label617;
        TLabel *Label618;
        TLabel *Label619;
        TLabel *Label620;
        TLabel *Label621;
        TLabel *Label622;
        TLabel *Label623;
        TLabel *Label624;
        TButton *Button609;
        TButton *Button610;
        TButton *Button611;
        TButton *Button612;
        TButton *Button613;
        TButton *Button614;
        TButton *Button615;
        TButton *Button616;
        TButton *Button617;
        TButton *Button618;
        TButton *Button619;
        TButton *Button620;
        TButton *Button621;
        TButton *Button622;
        TButton *Button623;
        TButton *Button624;
        TGroupBox *GroupBox40;
        TLabel *Label625;
        TLabel *Label626;
        TLabel *Label627;
        TLabel *Label628;
        TLabel *Label629;
        TLabel *Label630;
        TLabel *Label631;
        TLabel *Label632;
        TLabel *Label633;
        TLabel *Label634;
        TLabel *Label635;
        TLabel *Label636;
        TLabel *Label637;
        TLabel *Label638;
        TLabel *Label639;
        TLabel *Label640;
        TButton *Button625;
        TButton *Button626;
        TButton *Button627;
        TButton *Button628;
        TButton *Button629;
        TButton *Button630;
        TButton *Button631;
        TButton *Button632;
        TButton *Button633;
        TButton *Button634;
        TButton *Button635;
        TButton *Button636;
        TButton *Button637;
        TButton *Button638;
        TButton *Button639;
        TButton *Button640;
        TGroupBox *GroupBox51;
        TLabel *Label801;
        TLabel *Label802;
        TLabel *Label803;
        TLabel *Label804;
        TLabel *Label805;
        TLabel *Label806;
        TLabel *Label807;
        TLabel *Label808;
        TLabel *Label809;
        TLabel *Label810;
        TLabel *Label811;
        TLabel *Label812;
        TLabel *Label813;
        TLabel *Label814;
        TLabel *Label815;
        TLabel *Label816;
        TButton *Button801;
        TButton *Button802;
        TButton *Button803;
        TButton *Button804;
        TButton *Button805;
        TButton *Button806;
        TButton *Button807;
        TButton *Button808;
        TButton *Button809;
        TButton *Button810;
        TButton *Button811;
        TButton *Button812;
        TButton *Button813;
        TButton *Button814;
        TButton *Button815;
        TButton *Button816;
        TGroupBox *GroupBox52;
        TLabel *Label817;
        TLabel *Label818;
        TLabel *Label819;
        TLabel *Label820;
        TLabel *Label821;
        TLabel *Label822;
        TLabel *Label823;
        TLabel *Label824;
        TLabel *Label825;
        TLabel *Label826;
        TLabel *Label827;
        TLabel *Label828;
        TLabel *Label829;
        TLabel *Label830;
        TLabel *Label831;
        TLabel *Label832;
        TButton *Button817;
        TButton *Button818;
        TButton *Button819;
        TButton *Button820;
        TButton *Button821;
        TButton *Button822;
        TButton *Button823;
        TButton *Button824;
        TButton *Button825;
        TButton *Button826;
        TButton *Button827;
        TButton *Button828;
        TButton *Button829;
        TButton *Button830;
        TButton *Button831;
        TButton *Button832;
        TGroupBox *GroupBox53;
        TLabel *Label833;
        TLabel *Label834;
        TLabel *Label835;
        TLabel *Label836;
        TLabel *Label837;
        TLabel *Label838;
        TLabel *Label839;
        TLabel *Label840;
        TLabel *Label841;
        TLabel *Label842;
        TLabel *Label843;
        TLabel *Label844;
        TLabel *Label845;
        TLabel *Label846;
        TLabel *Label847;
        TLabel *Label848;
        TButton *Button833;
        TButton *Button834;
        TButton *Button835;
        TButton *Button836;
        TButton *Button837;
        TButton *Button838;
        TButton *Button839;
        TButton *Button840;
        TButton *Button841;
        TButton *Button842;
        TButton *Button843;
        TButton *Button844;
        TButton *Button845;
        TButton *Button846;
        TButton *Button847;
        TButton *Button848;
        TGroupBox *GroupBox54;
        TLabel *Label849;
        TLabel *Label850;
        TLabel *Label851;
        TLabel *Label852;
        TLabel *Label853;
        TLabel *Label854;
        TLabel *Label855;
        TLabel *Label856;
        TLabel *Label857;
        TLabel *Label858;
        TLabel *Label859;
        TLabel *Label860;
        TLabel *Label861;
        TLabel *Label862;
        TLabel *Label863;
        TLabel *Label864;
        TButton *Button849;
        TButton *Button850;
        TButton *Button851;
        TButton *Button852;
        TButton *Button853;
        TButton *Button854;
        TButton *Button855;
        TButton *Button856;
        TButton *Button857;
        TButton *Button858;
        TButton *Button859;
        TButton *Button860;
        TButton *Button861;
        TButton *Button862;
        TButton *Button863;
        TButton *Button864;
        TGroupBox *GroupBox55;
        TLabel *Label865;
        TLabel *Label866;
        TLabel *Label867;
        TLabel *Label868;
        TLabel *Label869;
        TLabel *Label870;
        TLabel *Label871;
        TLabel *Label872;
        TLabel *Label873;
        TLabel *Label874;
        TLabel *Label875;
        TLabel *Label876;
        TLabel *Label877;
        TLabel *Label878;
        TLabel *Label879;
        TLabel *Label880;
        TButton *Button865;
        TButton *Button866;
        TButton *Button867;
        TButton *Button868;
        TButton *Button869;
        TButton *Button870;
        TButton *Button871;
        TButton *Button872;
        TButton *Button873;
        TButton *Button874;
        TButton *Button875;
        TButton *Button876;
        TButton *Button877;
        TButton *Button878;
        TButton *Button879;
        TButton *Button880;
        TGroupBox *GroupBox56;
        TLabel *Label881;
        TLabel *Label882;
        TLabel *Label883;
        TLabel *Label884;
        TLabel *Label885;
        TLabel *Label886;
        TLabel *Label887;
        TLabel *Label888;
        TLabel *Label889;
        TLabel *Label890;
        TLabel *Label891;
        TLabel *Label892;
        TLabel *Label893;
        TLabel *Label894;
        TLabel *Label895;
        TLabel *Label896;
        TButton *Button881;
        TButton *Button882;
        TButton *Button883;
        TButton *Button884;
        TButton *Button885;
        TButton *Button886;
        TButton *Button887;
        TButton *Button888;
        TButton *Button889;
        TButton *Button890;
        TButton *Button891;
        TButton *Button892;
        TButton *Button893;
        TButton *Button894;
        TButton *Button895;
        TButton *Button896;
        TGroupBox *GroupBox57;
        TLabel *Label897;
        TLabel *Label898;
        TLabel *Label899;
        TLabel *Label900;
        TLabel *Label901;
        TLabel *Label902;
        TLabel *Label903;
        TLabel *Label904;
        TLabel *Label905;
        TLabel *Label906;
        TLabel *Label907;
        TLabel *Label908;
        TLabel *Label909;
        TLabel *Label910;
        TLabel *Label911;
        TLabel *Label912;
        TButton *Button897;
        TButton *Button898;
        TButton *Button899;
        TButton *Button900;
        TButton *Button901;
        TButton *Button902;
        TButton *Button903;
        TButton *Button904;
        TButton *Button905;
        TButton *Button906;
        TButton *Button907;
        TButton *Button908;
        TButton *Button909;
        TButton *Button910;
        TButton *Button911;
        TButton *Button912;
        TGroupBox *GroupBox58;
        TLabel *Label913;
        TLabel *Label914;
        TLabel *Label915;
        TLabel *Label916;
        TLabel *Label917;
        TLabel *Label918;
        TLabel *Label919;
        TLabel *Label920;
        TLabel *Label921;
        TLabel *Label922;
        TLabel *Label923;
        TLabel *Label924;
        TLabel *Label925;
        TLabel *Label926;
        TLabel *Label927;
        TLabel *Label928;
        TButton *Button913;
        TButton *Button914;
        TButton *Button915;
        TButton *Button916;
        TButton *Button917;
        TButton *Button918;
        TButton *Button919;
        TButton *Button920;
        TButton *Button921;
        TButton *Button922;
        TButton *Button923;
        TButton *Button924;
        TButton *Button925;
        TButton *Button926;
        TButton *Button927;
        TButton *Button928;
        TGroupBox *GroupBox59;
        TLabel *Label929;
        TLabel *Label930;
        TLabel *Label931;
        TLabel *Label932;
        TLabel *Label933;
        TLabel *Label934;
        TLabel *Label935;
        TLabel *Label936;
        TLabel *Label937;
        TLabel *Label938;
        TLabel *Label939;
        TLabel *Label940;
        TLabel *Label941;
        TLabel *Label942;
        TLabel *Label943;
        TLabel *Label944;
        TButton *Button929;
        TButton *Button930;
        TButton *Button931;
        TButton *Button932;
        TButton *Button933;
        TButton *Button934;
        TButton *Button935;
        TButton *Button936;
        TButton *Button937;
        TButton *Button938;
        TButton *Button939;
        TButton *Button940;
        TButton *Button941;
        TButton *Button942;
        TButton *Button943;
        TButton *Button944;
        TGroupBox *GroupBox60;
        TLabel *Label945;
        TLabel *Label946;
        TLabel *Label947;
        TLabel *Label948;
        TLabel *Label949;
        TLabel *Label950;
        TLabel *Label951;
        TLabel *Label952;
        TLabel *Label953;
        TLabel *Label954;
        TLabel *Label955;
        TLabel *Label956;
        TLabel *Label957;
        TLabel *Label958;
        TLabel *Label959;
        TLabel *Label960;
        TButton *Button945;
        TButton *Button946;
        TButton *Button947;
        TButton *Button948;
        TButton *Button949;
        TButton *Button950;
        TButton *Button951;
        TButton *Button952;
        TButton *Button953;
        TButton *Button954;
        TButton *Button955;
        TButton *Button956;
        TButton *Button957;
        TButton *Button958;
        TButton *Button959;
        TButton *Button960;
        TGroupBox *GroupBox71;
        TLabel *Label1121;
        TLabel *Label1122;
        TLabel *Label1123;
        TLabel *Label1124;
        TLabel *Label1125;
        TLabel *Label1126;
        TLabel *Label1127;
        TLabel *Label1128;
        TLabel *Label1129;
        TLabel *Label1130;
        TLabel *Label1131;
        TLabel *Label1132;
        TLabel *Label1133;
        TLabel *Label1134;
        TLabel *Label1135;
        TLabel *Label1136;
        TButton *Button1121;
        TButton *Button1122;
        TButton *Button1123;
        TButton *Button1124;
        TButton *Button1125;
        TButton *Button1126;
        TButton *Button1127;
        TButton *Button1128;
        TButton *Button1129;
        TButton *Button1130;
        TButton *Button1131;
        TButton *Button1132;
        TButton *Button1133;
        TButton *Button1134;
        TButton *Button1135;
        TButton *Button1136;
        TGroupBox *GroupBox72;
        TLabel *Label1137;
        TLabel *Label1138;
        TLabel *Label1139;
        TLabel *Label1140;
        TLabel *Label1141;
        TLabel *Label1142;
        TLabel *Label1143;
        TLabel *Label1144;
        TLabel *Label1145;
        TLabel *Label1146;
        TLabel *Label1147;
        TLabel *Label1148;
        TLabel *Label1149;
        TLabel *Label1150;
        TLabel *Label1151;
        TLabel *Label1152;
        TButton *Button1137;
        TButton *Button1138;
        TButton *Button1139;
        TButton *Button1140;
        TButton *Button1141;
        TButton *Button1142;
        TButton *Button1143;
        TButton *Button1144;
        TButton *Button1145;
        TButton *Button1146;
        TButton *Button1147;
        TButton *Button1148;
        TButton *Button1149;
        TButton *Button1150;
        TButton *Button1151;
        TButton *Button1152;
        TGroupBox *GroupBox73;
        TLabel *Label1153;
        TLabel *Label1154;
        TLabel *Label1155;
        TLabel *Label1156;
        TLabel *Label1157;
        TLabel *Label1158;
        TLabel *Label1159;
        TLabel *Label1160;
        TLabel *Label1161;
        TLabel *Label1162;
        TLabel *Label1163;
        TLabel *Label1164;
        TLabel *Label1165;
        TLabel *Label1166;
        TLabel *Label1167;
        TLabel *Label1168;
        TButton *Button1153;
        TButton *Button1154;
        TButton *Button1155;
        TButton *Button1156;
        TButton *Button1157;
        TButton *Button1158;
        TButton *Button1159;
        TButton *Button1160;
        TButton *Button1161;
        TButton *Button1162;
        TButton *Button1163;
        TButton *Button1164;
        TButton *Button1165;
        TButton *Button1166;
        TButton *Button1167;
        TButton *Button1168;
        TGroupBox *GroupBox74;
        TLabel *Label1169;
        TLabel *Label1170;
        TLabel *Label1171;
        TLabel *Label1172;
        TLabel *Label1173;
        TLabel *Label1174;
        TLabel *Label1175;
        TLabel *Label1176;
        TLabel *Label1177;
        TLabel *Label1178;
        TLabel *Label1179;
        TLabel *Label1180;
        TLabel *Label1181;
        TLabel *Label1182;
        TLabel *Label1183;
        TLabel *Label1184;
        TButton *Button1169;
        TButton *Button1170;
        TButton *Button1171;
        TButton *Button1172;
        TButton *Button1173;
        TButton *Button1174;
        TButton *Button1175;
        TButton *Button1176;
        TButton *Button1177;
        TButton *Button1178;
        TButton *Button1179;
        TButton *Button1180;
        TButton *Button1181;
        TButton *Button1182;
        TButton *Button1183;
        TButton *Button1184;
        TGroupBox *GroupBox75;
        TLabel *Label1185;
        TLabel *Label1186;
        TLabel *Label1187;
        TLabel *Label1188;
        TLabel *Label1189;
        TLabel *Label1190;
        TLabel *Label1191;
        TLabel *Label1192;
        TLabel *Label1193;
        TLabel *Label1194;
        TLabel *Label1195;
        TLabel *Label1196;
        TLabel *Label1197;
        TLabel *Label1198;
        TLabel *Label1199;
        TLabel *Label1200;
        TButton *Button1185;
        TButton *Button1186;
        TButton *Button1187;
        TButton *Button1188;
        TButton *Button1189;
        TButton *Button1190;
        TButton *Button1191;
        TButton *Button1192;
        TButton *Button1193;
        TButton *Button1194;
        TButton *Button1195;
        TButton *Button1196;
        TButton *Button1197;
        TButton *Button1198;
        TButton *Button1199;
        TButton *Button1200;
        TGroupBox *GroupBox76;
        TLabel *Label1201;
        TLabel *Label1202;
        TLabel *Label1203;
        TLabel *Label1204;
        TLabel *Label1205;
        TLabel *Label1206;
        TLabel *Label1207;
        TLabel *Label1208;
        TLabel *Label1209;
        TLabel *Label1210;
        TLabel *Label1211;
        TLabel *Label1212;
        TLabel *Label1213;
        TLabel *Label1214;
        TLabel *Label1215;
        TLabel *Label1216;
        TButton *Button1201;
        TButton *Button1202;
        TButton *Button1203;
        TButton *Button1204;
        TButton *Button1205;
        TButton *Button1206;
        TButton *Button1207;
        TButton *Button1208;
        TButton *Button1209;
        TButton *Button1210;
        TButton *Button1211;
        TButton *Button1212;
        TButton *Button1213;
        TButton *Button1214;
        TButton *Button1215;
        TButton *Button1216;
        TGroupBox *GroupBox77;
        TLabel *Label1217;
        TLabel *Label1218;
        TLabel *Label1219;
        TLabel *Label1220;
        TLabel *Label1221;
        TLabel *Label1222;
        TLabel *Label1223;
        TLabel *Label1224;
        TLabel *Label1225;
        TLabel *Label1226;
        TLabel *Label1227;
        TLabel *Label1228;
        TLabel *Label1229;
        TLabel *Label1230;
        TLabel *Label1231;
        TLabel *Label1232;
        TButton *Button1217;
        TButton *Button1218;
        TButton *Button1219;
        TButton *Button1220;
        TButton *Button1221;
        TButton *Button1222;
        TButton *Button1223;
        TButton *Button1224;
        TButton *Button1225;
        TButton *Button1226;
        TButton *Button1227;
        TButton *Button1228;
        TButton *Button1229;
        TButton *Button1230;
        TButton *Button1231;
        TButton *Button1232;
        TGroupBox *GroupBox78;
        TLabel *Label1233;
        TLabel *Label1234;
        TLabel *Label1235;
        TLabel *Label1236;
        TLabel *Label1237;
        TLabel *Label1238;
        TLabel *Label1239;
        TLabel *Label1240;
        TLabel *Label1241;
        TLabel *Label1242;
        TLabel *Label1243;
        TLabel *Label1244;
        TLabel *Label1245;
        TLabel *Label1246;
        TLabel *Label1247;
        TLabel *Label1248;
        TButton *Button1233;
        TButton *Button1234;
        TButton *Button1235;
        TButton *Button1236;
        TButton *Button1237;
        TButton *Button1238;
        TButton *Button1239;
        TButton *Button1240;
        TButton *Button1241;
        TButton *Button1242;
        TButton *Button1243;
        TButton *Button1244;
        TButton *Button1245;
        TButton *Button1246;
        TButton *Button1247;
        TButton *Button1248;
        TGroupBox *GroupBox79;
        TLabel *Label1249;
        TLabel *Label1250;
        TLabel *Label1251;
        TLabel *Label1252;
        TLabel *Label1253;
        TLabel *Label1254;
        TLabel *Label1255;
        TLabel *Label1256;
        TLabel *Label1257;
        TLabel *Label1258;
        TLabel *Label1259;
        TLabel *Label1260;
        TLabel *Label1261;
        TLabel *Label1262;
        TLabel *Label1263;
        TLabel *Label1264;
        TButton *Button1249;
        TButton *Button1250;
        TButton *Button1251;
        TButton *Button1252;
        TButton *Button1253;
        TButton *Button1254;
        TButton *Button1255;
        TButton *Button1256;
        TButton *Button1257;
        TButton *Button1258;
        TButton *Button1259;
        TButton *Button1260;
        TButton *Button1261;
        TButton *Button1262;
        TButton *Button1263;
        TButton *Button1264;
        TGroupBox *GroupBox80;
        TLabel *Label1265;
        TLabel *Label1266;
        TLabel *Label1267;
        TLabel *Label1268;
        TLabel *Label1269;
        TLabel *Label1270;
        TLabel *Label1271;
        TLabel *Label1272;
        TLabel *Label1273;
        TLabel *Label1274;
        TLabel *Label1275;
        TLabel *Label1276;
        TLabel *Label1277;
        TLabel *Label1278;
        TLabel *Label1279;
        TLabel *Label1280;
        TButton *Button1265;
        TButton *Button1266;
        TButton *Button1267;
        TButton *Button1268;
        TButton *Button1269;
        TButton *Button1270;
        TButton *Button1271;
        TButton *Button1272;
        TButton *Button1273;
        TButton *Button1274;
        TButton *Button1275;
        TButton *Button1276;
        TButton *Button1277;
        TButton *Button1278;
        TButton *Button1279;
        TButton *Button1280;
        TGroupBox *GroupBox91;
        TLabel *Label1441;
        TLabel *Label1442;
        TLabel *Label1443;
        TLabel *Label1444;
        TLabel *Label1445;
        TLabel *Label1446;
        TLabel *Label1447;
        TLabel *Label1448;
        TLabel *Label1449;
        TLabel *Label1450;
        TLabel *Label1451;
        TLabel *Label1452;
        TLabel *Label1453;
        TLabel *Label1454;
        TLabel *Label1455;
        TLabel *Label1456;
        TButton *Button1441;
        TButton *Button1442;
        TButton *Button1443;
        TButton *Button1444;
        TButton *Button1445;
        TButton *Button1446;
        TButton *Button1447;
        TButton *Button1448;
        TButton *Button1449;
        TButton *Button1450;
        TButton *Button1451;
        TButton *Button1452;
        TButton *Button1453;
        TButton *Button1454;
        TButton *Button1455;
        TButton *Button1456;
        TGroupBox *GroupBox92;
        TLabel *Label1457;
        TLabel *Label1458;
        TLabel *Label1459;
        TLabel *Label1460;
        TLabel *Label1461;
        TLabel *Label1462;
        TLabel *Label1463;
        TLabel *Label1464;
        TLabel *Label1465;
        TLabel *Label1466;
        TLabel *Label1467;
        TLabel *Label1468;
        TLabel *Label1469;
        TLabel *Label1470;
        TLabel *Label1471;
        TLabel *Label1472;
        TButton *Button1457;
        TButton *Button1458;
        TButton *Button1459;
        TButton *Button1460;
        TButton *Button1461;
        TButton *Button1462;
        TButton *Button1463;
        TButton *Button1464;
        TButton *Button1465;
        TButton *Button1466;
        TButton *Button1467;
        TButton *Button1468;
        TButton *Button1469;
        TButton *Button1470;
        TButton *Button1471;
        TButton *Button1472;
        TGroupBox *GroupBox93;
        TLabel *Label1473;
        TLabel *Label1474;
        TLabel *Label1475;
        TLabel *Label1476;
        TLabel *Label1477;
        TLabel *Label1478;
        TLabel *Label1479;
        TLabel *Label1480;
        TLabel *Label1481;
        TLabel *Label1482;
        TLabel *Label1483;
        TLabel *Label1484;
        TLabel *Label1485;
        TLabel *Label1486;
        TLabel *Label1487;
        TLabel *Label1488;
        TButton *Button1473;
        TButton *Button1474;
        TButton *Button1475;
        TButton *Button1476;
        TButton *Button1477;
        TButton *Button1478;
        TButton *Button1479;
        TButton *Button1480;
        TButton *Button1481;
        TButton *Button1482;
        TButton *Button1483;
        TButton *Button1484;
        TButton *Button1485;
        TButton *Button1486;
        TButton *Button1487;
        TButton *Button1488;
        TGroupBox *GroupBox94;
        TLabel *Label1489;
        TLabel *Label1490;
        TLabel *Label1491;
        TLabel *Label1492;
        TLabel *Label1493;
        TLabel *Label1494;
        TLabel *Label1495;
        TLabel *Label1496;
        TLabel *Label1497;
        TLabel *Label1498;
        TLabel *Label1499;
        TLabel *Label1500;
        TLabel *Label1501;
        TLabel *Label1502;
        TLabel *Label1503;
        TLabel *Label1504;
        TButton *Button1489;
        TButton *Button1490;
        TButton *Button1491;
        TButton *Button1492;
        TButton *Button1493;
        TButton *Button1494;
        TButton *Button1495;
        TButton *Button1496;
        TButton *Button1497;
        TButton *Button1498;
        TButton *Button1499;
        TButton *Button1500;
        TButton *Button1501;
        TButton *Button1502;
        TButton *Button1503;
        TButton *Button1504;
        TGroupBox *GroupBox95;
        TLabel *Label1505;
        TLabel *Label1506;
        TLabel *Label1507;
        TLabel *Label1508;
        TLabel *Label1509;
        TLabel *Label1510;
        TLabel *Label1511;
        TLabel *Label1512;
        TLabel *Label1513;
        TLabel *Label1514;
        TLabel *Label1515;
        TLabel *Label1516;
        TLabel *Label1517;
        TLabel *Label1518;
        TLabel *Label1519;
        TLabel *Label1520;
        TButton *Button1505;
        TButton *Button1506;
        TButton *Button1507;
        TButton *Button1508;
        TButton *Button1509;
        TButton *Button1510;
        TButton *Button1511;
        TButton *Button1512;
        TButton *Button1513;
        TButton *Button1514;
        TButton *Button1515;
        TButton *Button1516;
        TButton *Button1517;
        TButton *Button1518;
        TButton *Button1519;
        TButton *Button1520;
        TGroupBox *GroupBox96;
        TLabel *Label1521;
        TLabel *Label1522;
        TLabel *Label1523;
        TLabel *Label1524;
        TLabel *Label1525;
        TLabel *Label1526;
        TLabel *Label1527;
        TLabel *Label1528;
        TLabel *Label1529;
        TLabel *Label1530;
        TLabel *Label1531;
        TLabel *Label1532;
        TLabel *Label1533;
        TLabel *Label1534;
        TLabel *Label1535;
        TLabel *Label1536;
        TButton *Button1521;
        TButton *Button1522;
        TButton *Button1523;
        TButton *Button1524;
        TButton *Button1525;
        TButton *Button1526;
        TButton *Button1527;
        TButton *Button1528;
        TButton *Button1529;
        TButton *Button1530;
        TButton *Button1531;
        TButton *Button1532;
        TButton *Button1533;
        TButton *Button1534;
        TButton *Button1535;
        TButton *Button1536;
        TGroupBox *GroupBox97;
        TLabel *Label1537;
        TLabel *Label1538;
        TLabel *Label1539;
        TLabel *Label1540;
        TLabel *Label1541;
        TLabel *Label1542;
        TLabel *Label1543;
        TLabel *Label1544;
        TLabel *Label1545;
        TLabel *Label1546;
        TLabel *Label1547;
        TLabel *Label1548;
        TLabel *Label1549;
        TLabel *Label1550;
        TLabel *Label1551;
        TLabel *Label1552;
        TButton *Button1537;
        TButton *Button1538;
        TButton *Button1539;
        TButton *Button1540;
        TButton *Button1541;
        TButton *Button1542;
        TButton *Button1543;
        TButton *Button1544;
        TButton *Button1545;
        TButton *Button1546;
        TButton *Button1547;
        TButton *Button1548;
        TButton *Button1549;
        TButton *Button1550;
        TButton *Button1551;
        TButton *Button1552;
        TGroupBox *GroupBox98;
        TLabel *Label1553;
        TLabel *Label1554;
        TLabel *Label1555;
        TLabel *Label1556;
        TLabel *Label1557;
        TLabel *Label1558;
        TLabel *Label1559;
        TLabel *Label1560;
        TLabel *Label1561;
        TLabel *Label1562;
        TLabel *Label1563;
        TLabel *Label1564;
        TLabel *Label1565;
        TLabel *Label1566;
        TLabel *Label1567;
        TLabel *Label1568;
        TButton *Button1553;
        TButton *Button1554;
        TButton *Button1555;
        TButton *Button1556;
        TButton *Button1557;
        TButton *Button1558;
        TButton *Button1559;
        TButton *Button1560;
        TButton *Button1561;
        TButton *Button1562;
        TButton *Button1563;
        TButton *Button1564;
        TButton *Button1565;
        TButton *Button1566;
        TButton *Button1567;
        TButton *Button1568;
        TGroupBox *GroupBox99;
        TLabel *Label1569;
        TLabel *Label1570;
        TLabel *Label1571;
        TLabel *Label1572;
        TLabel *Label1573;
        TLabel *Label1574;
        TLabel *Label1575;
        TLabel *Label1576;
        TLabel *Label1577;
        TLabel *Label1578;
        TLabel *Label1579;
        TLabel *Label1580;
        TLabel *Label1581;
        TLabel *Label1582;
        TLabel *Label1583;
        TLabel *Label1584;
        TButton *Button1569;
        TButton *Button1570;
        TButton *Button1571;
        TButton *Button1572;
        TButton *Button1573;
        TButton *Button1574;
        TButton *Button1575;
        TButton *Button1576;
        TButton *Button1577;
        TButton *Button1578;
        TButton *Button1579;
        TButton *Button1580;
        TButton *Button1581;
        TButton *Button1582;
        TButton *Button1583;
        TButton *Button1584;
        TGroupBox *GroupBox100;
        TLabel *Label1585;
        TLabel *Label1586;
        TLabel *Label1587;
        TLabel *Label1588;
        TLabel *Label1589;
        TLabel *Label1590;
        TLabel *Label1591;
        TLabel *Label1592;
        TLabel *Label1593;
        TLabel *Label1594;
        TLabel *Label1595;
        TLabel *Label1596;
        TLabel *Label1597;
        TLabel *Label1598;
        TLabel *Label1599;
        TLabel *Label1600;
        TButton *Button1585;
        TButton *Button1586;
        TButton *Button1587;
        TButton *Button1588;
        TButton *Button1589;
        TButton *Button1590;
        TButton *Button1591;
        TButton *Button1592;
        TButton *Button1593;
        TButton *Button1594;
        TButton *Button1595;
        TButton *Button1596;
        TButton *Button1597;
        TButton *Button1598;
        TButton *Button1599;
        TButton *Button1600;
        TProgressBar *ProgressBar1;
        TTabSheet *TabSheet7;
        TMemo *Memo2;
        TButton *Script;
        TButton *Button1601;
        TButton *Button1602;
        TButton *Button1603;
        TComboBox *ComboBox1;
        TComboBox *ComboBox10;
        TLabel *Label1603;
        TComboBox *ComboBox11;
        TButton *Button1604;
        TButton *Button1605;
        TLabel *Label1604;
        TLabel *Label1605;
        TEdit *Edit5;
        TComboBox *ComboBox2;
        TComboBox *ComboBox3;
        TComboBox *ComboBox4;
        TLabel *Label1607;
        TShape *Shape1;
        TShape *Shape2;
        TShape *Shape3;
        TUpDown *UpDown1;
        TTimer *Timer2;
        TTimer *Timer3;
        TEdit *Edit8;
        TTimer *Timer4;
        TButton *Button1614;
        TButton *Button1615;
        TEdit *Edit9;
        TEdit *Edit6;
        TLabel *Label1602;
        TEdit *Edit7;
        TEdit *Edit10;
        TEdit *Edit11;
        TShape *Shape4;
        TShape *Shape5;
        TLabel *Label1606;
        TEdit *Edit12;
        TUpDown *UpDown2;
        TEdit *Edit13;
        TUpDown *UpDown3;
        TEdit *Edit14;
        TUpDown *UpDown4;
        TShape *Shape6;
        void __fastcall SendAllClick(TObject *Sender);
        void __fastcall Button1Click(TObject *Sender);
        void __fastcall GroupBox1Click(TObject *Sender);
        void __fastcall Save1Click(TObject *Sender);
        void __fastcall ConvertionClick(TObject *Sender);
        void __fastcall Open1Click(TObject *Sender);
        void __fastcall ReadAllClick(TObject *Sender);
        void __fastcall ScriptClick(TObject *Sender);
        void __fastcall Button1601Click(TObject *Sender);
        void __fastcall Button1602Click(TObject *Sender);
        void __fastcall Timer1Timer(TObject *Sender);
        void __fastcall ComboBox10Change(TObject *Sender);
        void __fastcall ComboBox11Change(TObject *Sender);
        void __fastcall Button1604Click(TObject *Sender);
        void __fastcall Button1605Click(TObject *Sender);
        void __fastcall ComboBox1Change(TObject *Sender);
        void __fastcall ComboBox2Change(TObject *Sender);
        void __fastcall ComboBox3Change(TObject *Sender);
        void __fastcall ComboBox4Change(TObject *Sender);
        void __fastcall Shape3MouseDown(TObject *Sender,
          TMouseButton Button, TShiftState Shift, int X, int Y);
        void __fastcall Shape2MouseDown(TObject *Sender,
          TMouseButton Button, TShiftState Shift, int X, int Y);
        void __fastcall UpDown1Click(TObject *Sender, TUDBtnType Button);
        void __fastcall UpDown2Click(TObject *Sender, TUDBtnType Button);
        void __fastcall UpDown3Click(TObject *Sender, TUDBtnType Button);
        void __fastcall UpDown4Click(TObject *Sender, TUDBtnType Button);
        void __fastcall Shape3ContextPopup(TObject *Sender,
          TPoint &MousePos, bool &Handled);
        void __fastcall Timer2Timer(TObject *Sender);
     //   void __fastcall Timer3Timer(TObject *Sender);
        void __fastcall Button1610Click(TObject *Sender);
        void __fastcall Shape2ContextPopup(TObject *Sender,
          TPoint &MousePos, bool &Handled);
        void __fastcall Timer4Timer(TObject *Sender);
        void __fastcall Button1614Click(TObject *Sender);
        void __fastcall PageControl1Change(TObject *Sender);
        void __fastcall Button1615Click(TObject *Sender);
        void __fastcall Edit9KeyPress(TObject *Sender, char &Key);
        void __fastcall Edit5KeyPress(TObject *Sender, char &Key);
        void __fastcall Edit12KeyPress(TObject *Sender, char &Key);
        void __fastcall Edit13KeyPress(TObject *Sender, char &Key);
        void __fastcall Edit14KeyPress(TObject *Sender, char &Key);

private:	// User declarations
public:		// User declarations
        __fastcall TForm1(TComponent* Owner);
};
//---------------------------------------------------------------------------
extern PACKAGE TForm1 *Form1;
//---------------------------------------------------------------------------
#endif
